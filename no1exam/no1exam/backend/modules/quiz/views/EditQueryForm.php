...
<section class="content">
    ...
    if (! empty($question_types)) {
        foreach ($question_types as $key => $value) {
            if (isset($questions[0]['question_type']) && ($questions[0]['question_type'] == $key)) {
                ?>
    ...
    if (! empty($education_levels)) {
        foreach ($education_levels as $key => $value) {
            if (isset($questions[0]['education_level']) && ($questions[0]['education_level'] == $key)) {
                ?>
            <option value="<?php echo $key; ?>" selected><?php echo $value; ?></option>
            <?php
            } else {
    ...
    if (! empty($years)) {
        foreach ($years as $key => $value) {
            if (isset($questions[0]['year']) && ($questions[0]['year'] == $key)) {
                ?>
            <option value="<?php echo $key; ?>" selected><?php echo $value; ?></option>
            <?php
            } else {
    ...
    ?>
        ...
        if (! empty($sems)) {
            foreach ($sems as $key => $value) {
                if (isset($questions[0]['sem']) && ($questions[0]['sem'] == $key)) {
                    ?>
                    <option value="<?php echo $key; ?>" selected><?php echo $value; ?></option>
                <?php
                } else {
    ...
    if ($question_levels) {
        foreach ($question_levels as $key => $value) {
            if (isset($questions[0]['question_level']) && ($questions[0]['question_level'] == $key)) {
                ?>
                <option value="<?php echo $key; ?>" selected><?php echo $value; ?></option>
                <?php
            } else {
			...
			<div class="form-group">
				<div class="col-md-12">
					Question <input type="text" class="form-control" id="question"
						...
						value="<?php echo isset($questions[0]['question']) ? $questions[0]['question'] : null; ?>" />
				</div>
			...
			<div class="form-group">
				<div class="col-md-6">
					Answer 1<input type="text" placeholder="Enter Answer"
						class="form-control" id="answer_1" name="answer_1"
						value="<?php echo isset($questions[0]['answer']) ? $questions[0]['answer'] : null; ?>"
						autocomplete="off" /> <label class="checkbox-inline"><input
						type="checkbox" value="" id="chk_answer_1" name="chk_answer_1"
						<?php echo isset($questions[0]['is_correct_answer']) ? ("true" == $questions[0]['is_correct_answer']  ? "checked" : "") : null; ?> />Correct
						Answer</label> <span id="err_answer_1"></span>
				</div>
				<div class="col-md-6">
					Answer 2<input type="text" placeholder="Enter Answer"
						class="form-control" id="answer_2" name="answer_2"
						value="<?php echo isset($questions[1]['answer']) ? $questions[1]['answer'] : null; ?>"
						autocomplete="off" /> <label class="checkbox-inline"><input
						type="checkbox" value="" id="chk_answer_2" name="chk_answer_2"
						<?php echo isset($questions[1]['is_correct_answer']) ? ("true" == $questions[1]['is_correct_answer']  ? "checked" : "") : null; ?> />Correct
			...
			<div class="form-group">
				<div class="col-md-6">
					Answer 3<input type="text" placeholder="Enter Answer"
						class="form-control" id="answer_3" name="answer_3"
						value="<?php echo isset($questions[2]['answer']) ? $questions[2]['answer'] : null; ?>"
						autocomplete="off" /> <label class="checkbox-inline"><input
						type="checkbox"
						value="<?php echo isset($questions[2]['is_correct_answer']) ? ("true" == $questions[2]['is_correct_answer']  ? "checked" : "") : null; ?>"
						id="chk_answer_3" name="chk_answer_3" />Correct Answer</label> <span
						id="err_answer_3"></span>
				</div>
				<div class="col-md-6">
					Answer 4<input type="text" placeholder="Enter Answer"
						class="form-control" id="answer_4" name="answer_4"
						value="<?php echo isset($questions[3]['answer']) ? $questions[3]['answer'] : null; ?>"
						autocomplete="off" /> <label class="checkbox-inline"><input
						type="checkbox"
						value="<?php echo isset($questions[3]['is_correct_answer']) ? ("true" == $questions[3]['is_correct_answer']  ? "checked" : "") : null; ?>"
						id="chk_answer_4" name="chk_answer_4" />Correct Answer</label> <span
						id="err_answer_4"></span>
				</div>
			</div>
			<div class="form-group">
...
toggleFile(1);
	...
	function saveQuestion(){
          ...
          $.post('<?php echo Yii::getAlias('@web').'/quiz/quiz/update-query';?>',objInputs,function(response){
              makeEmpty();
